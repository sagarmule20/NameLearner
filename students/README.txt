Put your student photos in THIS folder.
=======================================

File name pattern
-----------------

    Vorname_Zweitname_Drittname_Nachname_Geschlecht_Klasse.jpg

Six parts, separated by underscores. Use a single "-" for any part a student
does not have. Spaces are allowed *inside* a part.

Examples:

    Anna_-_-_Huber_f_3B.jpg                     -> Anna HUBER, 3B
    Lukas_Maria_-_Gruber_m_3B.jpg               -> Lukas Maria GRUBER, 3B
    Marie_Theresia_Anna_Pichler_f_4A.jpg        -> Marie Theresia Anna PICHLER, 4A
    Johannes_-_-_von Trapp_m_2C.jpg             -> Johannes VON TRAPP, 2C

Gender  : m / w / f / d   (used so the three wrong answers are the same gender)
Klasse  : anything you like - 3B, 4A, 2C, "Chor", ...
Formats : jpg, jpeg, png, webp, gif, avif, bmp, svg
          HEIC does NOT work - browsers cannot display it. Convert to JPG first.

Anything that does not match is listed under "Ubersprungene Dateien" in the
settings panel, together with the reason - so you can see exactly what to fix.

Privacy
-------

This folder is in .gitignore. The photos are never uploaded, never sent
anywhere, and never leave this computer. The app works completely offline.
